...
helper.get_version_string() {
	...
	if [ -z "$version_string" ]; then
		...
		helper.construct_version_matrix "$module_name" matrix_keys matrix_table
		helper.select_version "$module_name" matrix_keys matrix_table
...
helper.construct_version_matrix() {
	local module_name="$1"
